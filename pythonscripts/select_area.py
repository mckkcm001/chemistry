...
import types
import scraper
import practice_generator as pg
...
class simpleapp_tk(tk.Tk):
    ...
    def make_assess(self):   
        ...
        for t in teachers:
            for p in t[2]:
                ...
                pg.make_practice(t[0],roster,p,self.v_unit.get(),self.v_ass.get(),self.assess)
    ...
    def display_areas(self):
        self.AREAS = [(name,name) for _, name, _ in pkgutil.iter_modules(['problems'])]  
        ...
        self.label_area.grid(row=r, column=2)
    ...
    def display_skills(self,area):    
        ...
        mod = importlib.import_module('problems.'+area)
        ...
        for text, mode in SKILLS:
            ...
            self.skill_button[-1].grid(row=self.skill_rows, column=3)
            self.skill_rows +=1
